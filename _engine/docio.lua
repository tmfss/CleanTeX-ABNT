...
function doc.get_data(project)
    ...
    temp.presentation = project.data.presentation or "(Presentation não definido)"
...
function doc.input_project()
    ...
    local profile_name = profile_select
    if project.options.global.compile2abnt then
        profile_name = "abnt"
    end

    doc_table = merge_tables( doc.get_profile(project, profile_name) , doc.get_data(project) )
    doc_table.profile_name = profile_name
    doc_table = merge_tables( doc_table , project )
    ...
    local is_ptbr = (project.options.global.babel == "brazilian")
    if is_ptbr then
        doc_table.approval = "APROVADO"
        doc_table.assent = "ASSENTIMENTO"
        doc_table.acknowledgments_page = "Agradecimentos"
    else
        doc_table.approval = "APPROVED"
        doc_table.assent = "ASSENT"
        doc_table.acknowledgments_page = "Acknowledgments"
    end
